...
export default function () {
  ...
  const [instancePreviewInfo, setInstancePreviewInfo] = useState({
    avatarURL: "",
    tag: "",
    createdAt: "",
  });
  const [confirmInstanceModalAlert, setConfirmInstanceModalActive] =
    useState(false);
  ...
  async function checkToken() {
    ...
    const instanceInfo = await validateToken(
      token,
      !userAccountValue,
      disableTimeoutValue,
      noIntentsValue,
  ...
  function _addInstance() {
    ...
    addInstance({
      token,
      bot: !userAccountValue,
      timeoutDisabled: disableTimeoutValue,
      noIntents: noIntentsValue,
      ...instancePreviewInfo,
    });
  ...
  async function connectInstance(id) {
    ...
    const client = await createClient({
      ...
      disableTimeout: instance?.timeoutDisabled,
      noIntents: instance?.noIntents,
      bot: instance?.bot,
    });
        ...
        footer={
          <>
            ...
            <Button
              label="Cancel"
              size="md"
              onClick={() => setAddInstanceModalActive(false)}
            />
          </>
        }
      >
        ...
        <div className={styles.checkboxes}>
          <div className={styles.checkboxContainer}>
            <input
              ...
              type="checkbox"
              checked={userAccountValue}
              onChange={(e) => setUserAccountValue(e.target.checked)}
            />
          ...
          <div className={styles.checkboxContainer}>
            <input
              className={styles.checkbox}
              type="checkbox"
              checked={disableTimeoutValue}
              onChange={(e) => setDisableTimeoutValue(e.target.checked)}
            />
          ...
          <div className={styles.checkboxContainer}>
            <input
              className={styles.checkbox}
              type="checkbox"
              checked={noIntentsValue}
              onChange={(e) => setNoIntentsValue(e.target.checked)}
            />
            <h3 className={styles.label}>No Intents</h3>
          </div>
        </div>
        <Alert
          variant={addInstanceModalAlert.variant}
          description={<p>{addInstanceModalAlert.description}</p>}
          style={{
            marginTop: "1rem",
            display: addInstanceModalAlert.visible ? "flex" : "none",
          }}
        />
      </Modal>
        ...
        footer={
          <>
            <Button
              ...
              variant="primary"
              size="md"
              onClick={_addInstance}
            />
            <Button
              label="Cancel"
              size="md"
              onClick={() => setConfirmInstanceModalActive(false)}
            />
          </>
        }
      >
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          ...
          <Button
            ...
            onClick={() => {
              resetAlert(_addInstanceModalAlert);
              setToken("");
              setUserAccountValue(false);
              setDisableTimeoutValue(false);
              setNoIntentsValue(false);
              setAddInstanceModalActive(true);
            }}
          />
        </div>
      </div>
          ...
          <div className={styles.instance} key={instance.id}>
            ...
            <div className={styles.info}>
              <div className={styles.text}>
                ...
                <p className={styles.id}>
                  Type: <b>{instance.bot ? "Bot" : "User"}</b>
                </p>
                <p className={styles.id}>
                  Timeout: <b>{instance.timeoutDisabled ? "No" : "Yes"}</b>
                </p>
                <p className={styles.id}>
                  Intents: <b>{instance.noIntents ? "No" : "Yes"}</b>
                </p>
                <Alert
                  variant="warning"
                  description={
                  ...
                  style={{
                    marginTop: "1rem",
                    display: instanceAlerts[instance.id] ? "flex" : "none",
                  }}
                />
              </div>
              <div className={styles.buttons}>
                ...
                <Button
                  label="Delete"
                  iconLeft={faTrash}
                  customClass={styles.trashButton}
                  size="sm"
                  onClick={() => _removeInstance(instance.id)}
                />
                {copiedToken[instance.id] ? (
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      copyInstanceToken(instance.id, instance.token)
                    }
                  />
                )}
              </div>
