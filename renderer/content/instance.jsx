...
export default function () {
  ...
  useEffect(
    function () {
      client.on("guildCreate", function () {
        setContext((state) => ({
          ...state,
          instance: { ...state.instance, servers: client.guilds },
        }));
      });
      ...
      client.on("inviteCreate", async function (server) {
        ...
        try {
          invites = await server.getInvites();
        } catch {
          invites = null;
        }
    ...
    try {
        ...
        addLog(
          <p>
            ...
            <span style={{ color: "crimson" }}>
              Failed to fetch server (id: <b>{id}</b>)
            </span>
          </p>,
        );
      ...
      addLog(
        <p>
          ...
          <span style={{ color: "gold" }}>
            Selecting server... (this might take a while)
          </span>
        </p>,
      );
      ...
      try {
        templates = await server.getTemplates();
      } catch (error) {
        console.error(error);
        templates = null;
      }
  ...
  function convertStringToDate(timeString) {
    ...
    while ((match = timeRegex.exec(timeString)) !== null) {
      ...
      totalMilliseconds +=
        parseInt(timeValue) * getTimeUnitMultiplier(timeUnit);
    }
  ...
  const handleUserSelected = useCallback(
    (s) => setSelectedUsers(s.selectedRows),
    [],
  );

  const [selectedBanUsers, setSelectedBanUsers] = useState([]);
  const handleBanUserSelected = useCallback(
    (s) => setSelectedBanUsers(s.selectedRows),
    [],
  );

  const [selectedInvites, setSelectedInvites] = useState([]);
  const handleInviteSelected = useCallback(
    (s) => setSelectedInvites(s.selectedRows),
    [],
  );

  const [selectedTemplates, setSelectedTemplates] = useState([]);
  const handleTemplateSelected = useCallback(
    (s) => setSelectedTemplates(s.selectedRows),
    [],
  );

  const [selectedRemoveRoles, setSelectedRemoveRoles] = useState([]);
  const handleRemoveRoleSelected = useCallback(
    (s) => setSelectedRemoveRoles(s.selectedRows),
    [],
  );

  const [selectedAddRoles, setSelectedAddRoles] = useState([]);
  const handleAddRoleSelected = useCallback(
    (s) => setSelectedAddRoles(s.selectedRows),
    [],
  );

  const [selectedChannels, setSelectedChannels] = useState([]);
  const handleChannelSelected = useCallback(
    (s) => setSelectedChannels(s.selectedRows),
    [],
  );

  const [selectedRoles, setSelectedRoles] = useState([]);
  const handleRoleSelected = useCallback(
    (s) => setSelectedRoles(s.selectedRows),
    [],
  );

  const [selectedRemoveMembers, setSelectedRemoveMembers] = useState([]);
  const handleRemoveMemberSelected = useCallback(
    (s) => setSelectedRemoveMembers(s.selectedRows),
    [],
  );

  const [selectedAddMembers, setSelectedAddMembers] = useState([]);
  const handleAddMemberSelected = useCallback(
    (s) => setSelectedAddMembers(s.selectedRows),
    [],
  );

  const [selectedEmojis, setSelectedEmojis] = useState([]);
  const handleEmojiSelected = useCallback(
    (s) => setSelectedEmojis(s.selectedRows),
    [],
  );

  const [selectedStickers, setSelectedStickers] = useState([]);
  const handleStickerSelected = useCallback(
    (s) => setSelectedStickers(s.selectedRows),
    [],
  );
    ...
    try {
      ...
      const promises = members.map(function (member) {
        return new Promise(async function (resolve) {
          try {
            await member.edit(
              { nick: userNicknameValue },
              userNicknameReasonValue || undefined,
            );
            ...
            addLog(
              <p>
                ...
                <span style={{ color: "lightgreen" }}>
                  ...
                  to <b>"{userNicknameValue}"</b> (reason:{" "}
                  <b>"{userNicknameReasonValue}"</b>)
                </span>
              </p>,
            );
          } catch (error) {
      ...
      selectedUsers.forEach(function (user) {
        addLog(
          <p>
            ...
            <span style={{ color: "cyan" }}>[{selectedServer.name}]</span>&nbsp;
            <span style={{ color: "crimson" }}>
              Failed to nickname <b>{user.user}</b> to{" "}
              <b>"{userNicknameValue}"</b> (reason:{" "}
              <b>"{userNicknameReasonValue}"</b>)
            </span>
          </p>,
        );
        ...
        return new Promise(async function (resolve) {
          try {
            await member.edit(
              ...
              userTimeoutReasonValue || undefined,
            );
            ...
            addLog(
              <p>
                ...
                <span style={{ color: "lightgreen" }}>
                  ...
                  for <b>"{userTimeoutValue}"</b> (reason:{" "}
                  <b>"{userTimeoutReasonValue}"</b>)
                </span>
              </p>,
            );
          } catch (error) {
      ...
      selectedUsers.forEach(function (user) {
        addLog(
          <p>
            ...
            <span style={{ color: "cyan" }}>[{selectedServer.name}]</span>&nbsp;
            <span style={{ color: "crimson" }}>
              Failed to time out <b>{user.user}</b> for{" "}
              <b>"{userTimeoutValue}"</b> (reason:{" "}
              <b>"{userTimeoutReasonValue}"</b>)
            </span>
          </p>,
        );
            ...
            addLog(
              <p>
                ...
                <span style={{ color: "lightgreen" }}>
                  ...
                  (reason: <b>"{userKickReasonValue}"</b>)
                </span>
              </p>,
            );
          } catch (error) {
      ...
      selectedUsers.forEach(function (user) {
        addLog(
          <p>
            ...
            <span style={{ color: "cyan" }}>[{selectedServer.name}]</span>&nbsp;
            <span style={{ color: "crimson" }}>
              Failed to kick <b>{user.user}</b> (reason:{" "}
              <b>"{userKickReasonValue}"</b>)
            </span>
          </p>,
        );
      });
    ...
    try {
      ...
      const promises = members.map(function (member) {
        return new Promise(async function (resolve) {
          try {
            await member.ban(
              userBanValue && userBanValue !== "" ? parseInt(userBanValue) : 0,
              userBanReasonValue || undefined,
            );
            ...
            addLog(
              <p>
                ...
                <span style={{ color: "lightgreen" }}>
                  ...
                  and deleted messages from last{" "}
                  <b>
                    {userBanValue && userBanValue !== ""
                      ? parseInt(userBanValue)
                      : 0}
                  </b>{" "}
                  days (reason: <b>"{userBanReasonValue}"</b>)
                </span>
              </p>,
            );
          } catch (error) {
      ...
      selectedUsers.forEach(function (user) {
        addLog(
          <p>
            ...
            <span style={{ color: "cyan" }}>[{selectedServer.name}]</span>&nbsp;
            <span style={{ color: "crimson" }}>
              Failed to ban <b>{user.user}</b> and delete messages from last{" "}
              <b>
                {userBanValue && userBanValue !== ""
                  ? parseInt(userBanValue)
                  : 0}
  ...
  async function usersDM() {
    ...
    try {
      ...
      const promises = [
        ...Array(
          userDMAmountValue && userDMAmountValue !== ""
            ? parseInt(userDMAmountValue)
            : 1,
        ),
      ].map(() =>
      ...
      selectedUsers.forEach(function (user) {
        addLog(
          <p>
            ...
            <span style={{ color: "cyan" }}>[{selectedServer.name}]</span>&nbsp;
            <span style={{ color: "crimson" }}>
              Failed to DM <b>{user.user}</b> with <b>{userDMValue}</b>
            </span>
          </p>,
        );
      });
  ...
  async function serverRename() {
    ...
    try {
      await selectedServer?.edit(
        { name: serverNameValue },
        serverNameReasonValue || undefined,
      );
      ...
      addLog(
        <p>
          ...
          <span style={{ color: "cyan" }}>[{selectedServer.name}]</span>&nbsp;
          <span style={{ color: "lightgreen" }}>
            Renamed to <b>"{serverNameValue}"</b> (reason:{" "}
            <b>"{serverNameReasonValue}"</b>)
          </span>
        </p>,
      );
    } catch (error) {
      ...
      addLog(
        <p>
          ...
          <span style={{ color: "cyan" }}>[{selectedServer.name}]</span>&nbsp;
          <span style={{ color: "crimson" }}>
            Failed to rename to <b>"{serverNameValue}"</b> (reason:{" "}
            <b>"{serverNameReasonValue}"</b>)
          </span>
        </p>,
      );
    ...
    try {
      ...
      addLog(
        <p>
          ...
          <span style={{ color: "cyan" }}>[{selectedServer.name}]</span>&nbsp;
          <span style={{ color: "lightgreen" }}>
            Changed server icon to <b>"{response.name}"</b>
          </span>
        </p>,
      );
    } catch (error) {
  ...
  async function serverUnban() {
    ...
    const promises = selectedBanUsers.map(function (user) {
      return new Promise(async function (resolve) {
        try {
          await selectedServer?.unbanMember(
            user.id,
            serverUnbanReasonValue || undefined,
          );
          ...
          addLog(
            <p>
              ...
              &nbsp;
              <span style={{ color: "lightgreen" }}>
                Unbanned <b>{user.user}</b> (reason:{" "}
                <b>"{serverUnbanReasonValue}"</b>)
              </span>
            </p>,
          );
        } catch (error) {
          ...
          addLog(
            <p>
              ...
              &nbsp;
              <span style={{ color: "crimson" }}>
                Failed to unban <b>{user.user}</b> (reason:{" "}
                <b>"{serverUnbanReasonValue}"</b>)
              </span>
            </p>,
          );
    ...
    try {
      ...
      addLog(
        <p>
          ...
          &nbsp;
          <span style={{ color: "crimson" }}>
            Failed to copy bot invite link
          </span>
        </p>,
      );
    }
    ...
    try {
      ...
      addLog(
        <p>
          ...
          &nbsp;
          <span style={{ color: "crimson" }}>
            Failed to open bot invite link
          </span>
        </p>,
      );
    }
  ...
  async function serverPrune() {
    ...
    try {
      await selectedServer?.pruneMembers({
        days:
          serverPruneValue && serverPruneValue !== ""
            ? parseInt(serverPruneValue)
            : 7,
        reason: serverPruneReasonValue || undefined,
      });
      ...
      addLog(
        <p>
          ...
          <span style={{ color: "lightgreen" }}>
            ...
            , reason: <b>"{serverPruneReasonValue}"</b>)
          </span>
        </p>,
      );
    } catch (error) {
  ...
  async function serverTemplatesRefresh() {
    try {
      ...
      try {
        templates = await selectedServer?.getTemplates();
      } catch (error) {
        console.error(error);
        templates = null;
      }
        ...
        try {
          ...
          addLog(
            <p>
              ...
              &nbsp;
              <span style={{ color: "lightgreen" }}>
                Synced template <b>"{template.name}"</b> (code:{" "}
                <b>"{template.id}"</b>)
              </span>
            </p>,
          );
          ...
          addLog(
            <p>
              ...
              &nbsp;
              <span style={{ color: "crimson" }}>
                Failed to sync template <b>"{template.name}"</b> (code:{" "}
                <b>"{template.id}"</b>)
              </span>
            </p>,
          );
        ...
        try {
          ...
          addLog(
            <p>
              ...
              &nbsp;
              <span style={{ color: "lightgreen" }}>
                Deleted template <b>"{template.name}"</b> (code:{" "}
                <b>"{template.id}"</b>)
              </span>
            </p>,
          );
          ...
          addLog(
            <p>
              ...
              &nbsp;
              <span style={{ color: "crimson" }}>
                Failed to delete template <b>"{template.name}"</b> (code:{" "}
                <b>"{template.id}"</b>)
              </span>
            </p>,
          );
  ...
  async function serverTemplatesCreate() {
    try {
      const template = await selectedServer?.createTemplate(
        `Tmp. ${moment(Date.now()).format("MM/YYYY HH:mm:ss")}`,
      );
      await serverTemplatesRefresh();
      ...
      addLog(
        <p>
          ...
          <span style={{ color: "cyan" }}>[{selectedServer.name}]</span>&nbsp;
          <span style={{ color: "lightgreen" }}>
            Created template <b>"{template.name}"</b> (code:{" "}
            <b>"{template.id}"</b>)
          </span>
        </p>,
      );
  ...
  const [serverInviteDeleteModalActive, setServerInviteDeleteModalActive] =
    useState(false);
  const [serverInviteDeleteReasonValue, setServerInviteDeleteReasonValue] =
    useState("");
        ...
        try {
          ...
          addLog(
            <p>
              ...
              &nbsp;
              <span style={{ color: "lightgreen" }}>
                Deleted invite <b>"{invite.id}"</b> (reason:{" "}
                <b>"{serverInviteDeleteReasonValue}"</b>)
              </span>
            </p>,
          );
        } catch (error) {
          ...
          addLog(
            <p>
              ...
              &nbsp;
              <span style={{ color: "crimson" }}>
                Failed to delete invite <b>"{invite.id}"</b> (reason:{" "}
                <b>"{serverInviteDeleteReasonValue}"</b>)
              </span>
            </p>,
          );
  ...
  const [userRemoveRoleModalActive, setUserRemoveRoleModalActive] =
    useState(false);
  const [userRemoveRoleReasonValue, setUserRemoveRoleReasonValue] =
    useState("");
            ...
            return new Promise(async function (resolve) {
              try {
                await member.removeRole(
                  role.id,
                  userRemoveRoleReasonValue || undefined,
                );
                ...
                addLog(
                  <p>
                    ...
                    &nbsp;
                    <span style={{ color: "lightgreen" }}>
                      Removed role <b>"{role.name}"</b> from member{" "}
                      <b>
                        {member.discriminator !== "0"
                          ? `${member.username}#${member.discriminator}`
                          : member.username}
                      </b>{" "}
                      (reason: <b>"{userRemoveRoleReasonValue}"</b>)
                    </span>
                  </p>,
                );
              } catch (error) {
                ...
                addLog(
                  <p>
                    ...
                    &nbsp;
                    <span style={{ color: "crimson" }}>
                      Failed to remove role <b>"{role.name}"</b> from member{" "}
                      <b>
                        {member.discriminator !== "0"
                          ? `${member.username}#${member.discriminator}`
                          : member.username}
      ...
      selectedUsers.forEach(function (member) {
        addLog(
          <p>
            ...
            <span style={{ color: "cyan" }}>[{selectedServer.name}]</span>&nbsp;
            <span style={{ color: "crimson" }}>
              Failed to remove <b>{selectedRemoveRoles.length}</b> roles from
              member{" "}
              <b>
                {member.discriminator !== "0"
                  ? `${member.username}#${member.discriminator}`
            ...
            return new Promise(async function (resolve) {
              try {
                await member.addRole(
                  role.id,
                  userAddRoleReasonValue || undefined,
                );
                ...
                addLog(
                  <p>
                    ...
                    &nbsp;
                    <span style={{ color: "lightgreen" }}>
                      Added role <b>"{role.name}"</b> to member{" "}
                      <b>
                        {member.discriminator !== "0"
                          ? `${member.username}#${member.discriminator}`
                          : member.username}
                      </b>{" "}
                      (reason: <b>"{userAddRoleReasonValue}"</b>)
                    </span>
                  </p>,
                );
              } catch (error) {
                ...
                addLog(
                  <p>
                    ...
                    &nbsp;
                    <span style={{ color: "crimson" }}>
                      Failed to add role <b>"{role.name}"</b> to member{" "}
                      <b>
                        {member.discriminator !== "0"
                          ? `${member.username}#${member.discriminator}`
                          : member.username}
      ...
      selectedUsers.forEach(function (member) {
        addLog(
          <p>
            ...
            <span style={{ color: "cyan" }}>[{selectedServer.name}]</span>&nbsp;
            <span style={{ color: "crimson" }}>
              Failed to add <b>{selectedAddRoles.length}</b> roles to member{" "}
              <b>
                {member.discriminator !== "0"
                  ? `${member.username}#${member.discriminator}`
                  : member.username}
  ...
  const [channelMessageModalActive, setChannelMessageModalActive] =
    useState(false);
  const [channelMessageValue, setChannelMessageValue] = useState("");
  const [channelMessageAmountValue, setChannelMessageAmountValue] =
    useState("");
  ...
  async function channelMessage() {
    ...
    try {
      const channels = await selectedServer?.channels
        ?.filter((channel) =>
          selectedChannels?.find((_channel) => channel?.id === _channel?.id),
        )
        ?.filter((channel) => channel?.createMessage);

      const promises = [
        ...Array(
          channelMessageAmountValue && channelMessageAmountValue !== ""
            ? parseInt(channelMessageAmountValue)
            : 1,
        ),
      ].map(() =>
            ...
            try {
              ...
              addLog(
                <p>
                  ...
                  &nbsp;
                  <span style={{ color: "lightgreen" }}>
                    Messaged in channel <b>"{channel.name}"</b>:{" "}
                    <b>{channelMessageValue}</b>
                  </span>
                </p>,
              );
            } catch (error) {
              ...
              addLog(
                <p>
                  ...
                  &nbsp;
                  <span style={{ color: "crimson" }}>
                    Failed to message in channel <b>"{channel.name}"</b>:{" "}
                    <b>{channelMessageValue}</b>
                  </span>
                </p>,
              );
      ...
      const promises = channels.map(function (channel) {
        return new Promise(async function (resolve) {
          try {
            await channel.purge({
              limit:
                channelPurgeValue && channelPurgeValue !== ""
                  ? parseInt(channelPurgeValue)
                  : -1,
              reason: channelPurgeReasonValue || undefined,
            });
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "lightgreen" }}>
                  Purged channel <b>"{channel.name}"</b> (limit:{" "}
                  {channelPurgeValue && channelPurgeValue !== ""
                    ? parseInt(channelPurgeValue)
                    : -1}
                  , reason: <b>"{channelPurgeReasonValue}"</b>)
                </span>
              </p>,
            );
          } catch (error) {
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "crimson" }}>
                  Failed to purge channel <b>"{channel.name}"</b> (limit:{" "}
                  {channelPurgeValue && channelPurgeValue !== ""
                    ? parseInt(channelPurgeValue)
                    : -1}
                  , reason: <b>"{channelPurgeReasonValue}"</b>)
  ...
  const [channelCreateInviteModalActive, setChannelCreateInviteModalActive] =
    useState(false);
  const [channelCreateInviteMaxAgeValue, setChannelCreateInviteMaxAgeValue] =
    useState("");
  const [channelCreateInviteMaxUsesValue, setChannelCreateInviteMaxUsesValue] =
    useState("");
  const [
    channelCreateInviteTemporaryValue,
    setChannelCreateInviteTemporaryValue,
  ] = useState(false);
  const [channelCreateInviteUniqueValue, setChannelCreateInviteUniqueValue] =
    useState(false);
  const [channelCreateInviteReasonValue, setChannelCreateInviteReasonValue] =
    useState(false);
        ...
        return new Promise(async function (resolve) {
          try {
            const invite = await channel.createInvite({
              maxAge: channelCreateInviteMaxAgeValue
                ? channelCreateInviteMaxAgeValue * 24 * 60 * 60
                : undefined,
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "lightgreen" }}>
                  Created invite for channel <b>"{channel.name}"</b> (code:{" "}
                  <b>"{invite.code}"</b>, max age:{" "}
                  <b>"{channelCreateInviteMaxAgeValue}"</b>, max uses:{" "}
                  <b>"{channelCreateInviteMaxUsesValue}"</b>, temporary?:{" "}
                  <b>"{channelCreateInviteTemporaryValue.toString()}"</b>,
                  unique?: <b>"{channelCreateInviteUniqueValue.toString()}"</b>,
                  reason: <b>"{channelCreateInviteReasonValue}"</b>)
                </span>
              </p>,
            );
          } catch (error) {
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "crimson" }}>
                  Failed to create invite for channel <b>"{channel.name}"</b>{" "}
                  (max age: <b>"{channelCreateInviteMaxAgeValue}"</b>, max uses:{" "}
                  <b>"{channelCreateInviteMaxUsesValue}"</b>, temporary?:{" "}
                  <b>"{channelCreateInviteTemporaryValue.toString()}"</b>,
                  unique?: <b>"{channelCreateInviteUniqueValue.toString()}"</b>,
                  reason: <b>"{channelCreateInviteReasonValue}"</b>)
      ...
      selectedChannels.forEach(function (channel) {
        addLog(
          <p>
            ...
            <span style={{ color: "cyan" }}>[{selectedServer.name}]</span>&nbsp;
            <span style={{ color: "crimson" }}>
              Failed to create invite for channel <b>"{channel.name}"</b> (max
              age: <b>"{channelCreateInviteMaxAgeValue}"</b>, max uses:{" "}
              <b>"{channelCreateInviteMaxUsesValue}"</b>, temporary?:{" "}
              <b>"{channelCreateInviteTemporaryValue.toString()}"</b>, unique?:{" "}
              <b>"{channelCreateInviteUniqueValue.toString()}"</b>, reason:{" "}
              <b>"{channelCreateInviteReasonValue}"</b>)
            </span>
          </p>,
        );
      });
  ...
  const [channelDeleteModalActive, setChannelDeleteModalActive] =
    useState(false);
          ...
          try {
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "lightgreen" }}>
                  Deleted channel <b>"{channel.name}"</b> (id:{" "}
                  <b>{channel.id}</b>, reason:{" "}
                  <b>"{channelDeleteReasonValue}"</b>)
                </span>
              </p>,
            );
          } catch (error) {
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "crimson" }}>
                  Failed to delete channel <b>"{channel.name}"</b> (id:{" "}
                  <b>{channel.id}</b>, reason:{" "}
                  <b>"{channelDeleteReasonValue}"</b>)
                </span>
              </p>,
      ...
      selectedChannels.forEach(function (channel) {
        addLog(
          <p>
            ...
            <span style={{ color: "crimson" }}>
              Failed to delete channel <b>"{channel.name}"</b> (id:{" "}
              <b>{channel.id}</b>, reason: <b>"{channelDeleteReasonValue}"</b>)
            </span>
          </p>,
        );
      });
  ...
  const [channelRenameModalActive, setChannelRenameModalActive] =
    useState(false);
      ...
      const promises = channels.map(function (channel) {
        return new Promise(async function (resolve) {
          try {
            ...
            await channel.edit(
              { name: channelRenameValue },
              channelRenameReasonValue || undefined,
            );
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "lightgreen" }}>
                  Renamed channel <b>"{channelName}"</b> to{" "}
                  <b>"{channel.name}"</b> (id: <b>{channel.id}</b>, reason:{" "}
                  <b>"{channelRenameReasonValue}"</b>)
                </span>
              </p>,
            );
          } catch (error) {
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "crimson" }}>
                  Failed to rename channel <b>"{channel.name}"</b> to{" "}
                  <b>"{channelRenameValue}"</b> (id: <b>{channel.id}</b>,
                  reason: <b>"{channelRenameReasonValue}"</b>)
                </span>
              </p>,
            );
          }
      ...
      selectedChannels.forEach(function (channel) {
        addLog(
          <p>
            ...
            <span style={{ color: "crimson" }}>
              Failed to rename channel <b>"{channel.name}"</b> to{" "}
              <b>"{channelRenameValue}"</b> (id: <b>{channel.id}</b>, reason:{" "}
              <b>"{channelRenameReasonValue}"</b>)
            </span>
          </p>,
        );
  ...
  const [channelCreateModalActive, setChannelCreateModalActive] =
    useState(false);
  ...
  async function channelCreate() {
    ...
    const promises = [
      ...Array(
        channelCreateAmountValue && channelCreateAmountValue !== ""
          ? parseInt(channelCreateAmountValue)
          : 1,
      ),
    ].map(
      () =>
        new Promise(async function (resolve) {
          try {
            const channel = await selectedServer?.createChannel(
              channelCreateNameValue,
              channelCreateTypeValue,
              {
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "lightgreen" }}>
                  Created channel <b>"{channelCreateNameValue}"</b> (id:{" "}
                  <b>{channel.id}</b>, type: <b>{channelCreateTypeValue}</b>,
                  nsfw: <b>{channelCreateNSFWValue.toString()}</b>, reason:{" "}
                  <b>"{channelCreateReasonValue}"</b>)
                </span>
              </p>,
            );
          } catch (error) {
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "crimson" }}>
                  Failed to create channel <b>"{channelCreateNameValue}"</b>{" "}
                  (type: <b>{channelCreateTypeValue}</b>, nsfw:{" "}
                  <b>{channelCreateNSFWValue.toString()}</b>, reason:{" "}
                  <b>"{channelCreateReasonValue}"</b>)
                </span>
              </p>,
            );
  ...
  async function roleDelete() {
    ...
    try {
      const roles = await selectedServer?.roles
        ?.filter((role) =>
          selectedRoles?.find((_role) => role?.id === _role?.id),
        )
          ...
          try {
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "lightgreen" }}>
                  Deleted role <b>"{role.name}"</b> (id: <b>{role.id}</b>,
                  reason: <b>"{roleDeleteReasonValue}"</b>)
                </span>
              </p>,
            );
          } catch (error) {
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "crimson" }}>
                  Failed to delete role <b>"{role.name}"</b> (id:{" "}
                  <b>{role.id}</b>, reason: <b>"{roleDeleteReasonValue}"</b>)
                </span>
              </p>,
            );
          }
      ...
      selectedRoles.forEach(function (role) {
        addLog(
          <p>
            ...
            <span style={{ color: "cyan" }}>[{selectedServer.name}]</span>&nbsp;
            <span style={{ color: "crimson" }}>
              Failed to delete role <b>"{role.name}"</b> (id: <b>{role.id}</b>,
              reason: <b>"{roleDeleteReasonValue}"</b>)
            </span>
          </p>,
        );
      ...
      const promises = roles.map(function (role) {
        return new Promise(async function (resolve) {
          try {
            ...
            await role.edit(
              { name: roleRenameValue },
              roleRenameReasonValue || undefined,
            );
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "lightgreen" }}>
                  Renamed role <b>"{roleName}"</b> to <b>"{role.name}"</b> (id:{" "}
                  <b>{role.id}</b>, reason: <b>"{roleRenameReasonValue}"</b>)
                </span>
              </p>,
            );
          } catch (error) {
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "crimson" }}>
                  Failed to rename role <b>"{role.name}"</b> to{" "}
                  <b>"{roleRenameValue}"</b> (id: <b>{role.id}</b>, reason:{" "}
                  <b>"{roleRenameReasonValue}"</b>)
                </span>
              </p>,
            );
          }
  ...
  const [roleCreateMentionableValue, setRoleCreateMentionableValue] =
    useState(false);
  ...
  async function roleCreate() {
    ...
    const promises = [
      ...Array(
        roleCreateAmountValue && roleCreateAmountValue !== ""
          ? parseInt(roleCreateAmountValue)
          : 1,
      ),
    ].map(
      () =>
          ...
          try {
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "lightgreen" }}>
                  Created role <b>"{roleCreateNameValue}"</b> (id:{" "}
                  <b>{role.id}</b>, hoist:{" "}
                  <b>{roleCreateHoistValue.toString()}</b>, mentionable:{" "}
                  <b>{roleCreateMentionableValue.toString()}</b>, reason:{" "}
                  <b>"{roleCreateReasonValue}"</b>)
                </span>
              </p>,
            );
          } catch (error) {
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "crimson" }}>
                  Failed to create role <b>"{roleCreateNameValue}"</b> (hoist:{" "}
                  <b>{roleCreateHoistValue.toString()}</b>, mentionable:{" "}
                  <b>{roleCreateMentionableValue.toString()}</b>, reason:{" "}
                  <b>"{roleCreateReasonValue}"</b>)
                </span>
  ...
  const [roleAddMemberModalActive, setRoleAddMemberModalActive] =
    useState(false);
          ...
          const rolePromises = selectedRoles.map(function (role) {
            return new Promise(async function (resolve) {
              try {
                await member.addRole(
                  role.id,
                  roleAddMemberReasonValue || undefined,
                );
                ...
                addLog(
                  <p>
                    ...
                    <span style={{ color: "lightgreen" }}>
                      ...
                      to role <b>{role.name}</b> (reason:{" "}
                      <b>"{roleAddMemberReasonValue}"</b>)
                    </span>
                  </p>,
                );
              } catch (error) {
  ...
  const [roleRemoveMemberModalActive, setRoleRemoveMemberModalActive] =
    useState(false);
  const [roleRemoveMemberReasonValue, setRoleRemoveMemberReasonValue] =
    useState("");
          ...
          const rolePromises = selectedRoles.map(function (role) {
            return new Promise(async function (resolve) {
              try {
                await member.removeRole(
                  role.id,
                  roleRemoveMemberReasonValue || undefined,
                );
                ...
                addLog(
                  <p>
                    ...
                    <span style={{ color: "lightgreen" }}>
                      ...
                      from role <b>{role.name}</b> (reason:{" "}
                      <b>"{roleRemoveMemberReasonValue}"</b>)
                    </span>
                  </p>,
                );
              } catch (error) {
      ...
      const promises = selectedEmojis.map(function (emoji) {
        return new Promise(async function (resolve) {
          try {
            ...
            const _emoji = await selectedServer?.editEmoji(
              emoji?.id,
              { name: emojiRenameValue },
              emojiRenameReasonValue || null,
            );
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "lightgreen" }}>
                  Renamed emoji <b>"{emojiName}"</b> to <b>"{_emoji.name}"</b>{" "}
                  (id: <b>{emoji.id}</b>, reason:{" "}
                  <b>"{emojiRenameReasonValue}"</b>)
                </span>
              </p>,
            );
          } catch (err) {
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "crimson" }}>
                  Failed to rename emoji <b>"{emoji.name}"</b> to{" "}
                  <b>"{emojiRenameValue}"</b> (id: <b>{emoji.id}</b>, reason:{" "}
                  <b>"{emojiRenameReasonValue}"</b>)
                </span>
              </p>,
            );
  ...
  const [stickerRenameModalActive, setStickerRenameModalActive] =
    useState(false);
      ...
      const promises = selectedStickers.map(function (sticker) {
        return new Promise(async function (resolve) {
          try {
            ...
            const _sticker = await selectedServer?.editSticker(
              sticker?.id,
              { name: stickerRenameValue },
              stickerRenameReasonValue || null,
            );
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "lightgreen" }}>
                  Renamed sticker <b>"{stickerName}"</b> to{" "}
                  <b>"{_sticker.name}"</b> (id: <b>{sticker.id}</b>, reason:{" "}
                  <b>"{stickerRenameReasonValue}"</b>)
                </span>
              </p>,
            );
          } catch (err) {
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "crimson" }}>
                  Failed to rename sticker <b>"{sticker.name}"</b> to{" "}
                  <b>"{stickerRenameValue}"</b> (id: <b>{sticker.id}</b>,
                  reason: <b>"{stickerRenameReasonValue}"</b>)
                </span>
              </p>,
            );
          }
      ...
      selectedStickers.forEach(function (sticker) {
        addLog(
          <p>
            ...
            <span style={{ color: "crimson" }}>
              Failed to rename sticker <b>"{sticker.name}"</b> to{" "}
              <b>"{stickerRenameValue}"</b> (id: <b>{sticker.id}</b>, reason:{" "}
              <b>"{stickerRenameReasonValue}"</b>)
            </span>
          </p>,
        );
  ...
  const [stickerDeleteModalActive, setStickerDeleteModalActive] =
    useState(false);
  ...
  async function stickerDelete() {
    ...
    try {
      const promises = selectedStickers.map(function (sticker) {
        return new Promise(async function (resolve) {
          try {
            await selectedServer?.deleteSticker(
              sticker.id,
              stickerDeleteReasonValue || undefined,
            );
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "lightgreen" }}>
                  Deleted sticker <b>"{sticker.name}"</b> (id:{" "}
                  <b>{sticker.id}</b>, reason:{" "}
                  <b>"{stickerDeleteReasonValue}"</b>)
                </span>
              </p>,
            );
          } catch (error) {
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "crimson" }}>
                  Failed to delete sticker <b>"{sticker.name}"</b> (id:{" "}
                  <b>{sticker.id}</b>, reason:{" "}
                  <b>"{stickerDeleteReasonValue}"</b>)
                </span>
              </p>,
      ...
      selectedStickers.forEach(function (sticker) {
        addLog(
          <p>
            ...
            <span style={{ color: "crimson" }}>
              Failed to delete sticker <b>"{sticker.name}"</b> (id:{" "}
              <b>{sticker.id}</b>, reason: <b>"{stickerDeleteReasonValue}"</b>)
            </span>
          </p>,
        );
      });
  ...
  async function emojiDelete() {
    ...
    try {
      const promises = selectedEmojis.map(function (emoji) {
        return new Promise(async function (resolve) {
          try {
            await selectedServer?.deleteEmoji(
              emoji.id,
              emojiDeleteReasonValue || undefined,
            );
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "lightgreen" }}>
                  Deleted emoji <b>"{emoji.name}"</b> (id: <b>{emoji.id}</b>,
                  reason: <b>"{emojiDeleteReasonValue}"</b>)
                </span>
              </p>,
            );
          } catch (error) {
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "crimson" }}>
                  Failed to delete emoji <b>"{emoji.name}"</b> (id:{" "}
                  <b>{emoji.id}</b>, reason: <b>"{emojiDeleteReasonValue}"</b>)
                </span>
              </p>,
            );
          }
  ...
  async function emojiCreate() {
    ...
    try {
      ...
      const promises = [
        ...Array(
          emojiCreateAmountValue && emojiCreateAmountValue !== ""
            ? parseInt(emojiCreateAmountValue)
            : 1,
        ),
      ].map(
        () =>
            ...
            try {
              ...
              addLog(
                <p>
                  ...
                  &nbsp;
                  <span style={{ color: "lightgreen" }}>
                    Created emoji <b>"{emojiCreateNameValue}"</b> (id:{" "}
                    <b>{emoji.id}</b>, reason: <b>"{emojiCreateReasonValue}"</b>
                    )
                  </span>
                </p>,
              );
              ...
              addLog(
                <p>
                  ...
                  &nbsp;
                  <span style={{ color: "crimson" }}>
                    Failed to create emoji <b>"{emojiCreateNameValue}"</b>{" "}
                    (reason: <b>"{emojiCreateReasonValue}"</b>)
                  </span>
                </p>,
              );
            }
      ...
      addLog(
        <p>
          ...
          <span style={{ color: "cyan" }}>[{selectedServer.name}]</span>&nbsp;
          <span style={{ color: "crimson" }}>
            Failed to create emoji <b>"{emojiCreateNameValue}"</b> (reason:{" "}
            <b>"{emojiCreateReasonValue}"</b>)
          </span>
        </p>,
      );
    }
  ...
  const [stickerCreateModalActive, setStickerCreateModalActive] =
    useState(false);
  ...
  async function stickerCreate() {
    ...
    try {
      ...
      const promises = [
        ...Array(
          stickerCreateAmountValue && stickerCreateAmountValue !== ""
            ? parseInt(stickerCreateAmountValue)
            : 1,
        ),
      ].map(
        () =>
            ...
            try {
              ...
              addLog(
                <p>
                  ...
                  &nbsp;
                  <span style={{ color: "lightgreen" }}>
                    Created sticker <b>"{stickerCreateNameValue}"</b> (id:{" "}
                    <b>{sticker.id}</b>, reason:{" "}
                    <b>"{stickerCreateReasonValue}"</b>)
                  </span>
                </p>,
              );
            } catch (error) {
              ...
              addLog(
                <p>
                  ...
                  &nbsp;
                  <span style={{ color: "crimson" }}>
                    Failed to create sticker <b>"{stickerCreateNameValue}"</b>{" "}
                    (reason: <b>"{stickerCreateReasonValue}"</b>)
                  </span>
                </p>,
              );
            }
      ...
      addLog(
        <p>
          ...
          <span style={{ color: "cyan" }}>[{selectedServer.name}]</span>&nbsp;
          <span style={{ color: "crimson" }}>
            Failed to create sticker <b>"{stickerCreateNameValue}"</b> (reason:{" "}
            <b>"{stickerCreateReasonValue}"</b>)
          </span>
        </p>,
      );
  ...
  async function serverBansRefresh() {
    try {
      ...
      try {
        bans = await selectedServer?.getBans();
      } catch (error) {
        console.error(error);
        bans = null;
      }
  ...
  async function serverInvitesRefresh() {
    try {
      ...
      try {
        invites = await selectedServer?.getInvites();
      } catch (error) {
        console.error(error);
        invites = null;
      }
  ...
  async function serverCreate() {
    ...
    const promises = [
      ...Array(
        serverCreateAmountValue && serverCreateAmountValue !== ""
          ? parseInt(serverCreateAmountValue)
          : 1,
      ),
    ].map(
      () =>
          ...
          try {
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "lightgreen" }}>
                  Created server <b>"{serverCreateNameValue}"</b> (id:{" "}
                  <b>{server.id}</b>)
                </span>
              </p>,
            );
            ...
            addLog(
              <p>
                ...
                &nbsp;
                <span style={{ color: "crimson" }}>
                  Failed to create server <b>"{serverCreateNameValue}"</b>
                </span>
              </p>,
            );
          }
  ...
  const [removeMembersFilterType, setRemoveMembersFilterType] =
    useState("username");
  ...
  const tabs = [
        ...
        <>
          ...
          {client.bot ? (
            <Alert
              ...
              description={
                <p>
                  Refreshing is not required on bot accounts unless for
                  templates; <b>only manually refresh if content is missing</b>.
                </p>
              }
            />
          ) : (
            <Alert
              ...
              description={
                <p>
                  Users, channels, etc do not automatically refresh on user
                  accounts; <b>manually click the refresh button</b>.
                </p>
              }
            />
          )}
          ...
          >
            <Input
              label="Name"
              value={serverCreateNameValue}
              onInput={(e) => setServerCreateNameValue(e.target.value)}
            />
            <div style={{ marginTop: "1rem" }}>
              <Input
                label="Amount"
                value={serverCreateAmountValue}
                onInput={(e) => setServerCreateAmountValue(e.target.value)}
              />
            </div>
          </Modal>

          <div
            className={`${styles.title} ${styles.topLabel}`}
            style={{ marginBottom: ".5rem" }}
          >
                  ...
                  {client?.bot && (
                    <>
                      <Button
                        ...
                        customClass={styles.button}
                        onClick={() => {
                          setServerCreateNameValue("");
                          setServerCreateAmountValue(1);
                          setServerCreateModalActive(true);
                        }}
                      />
                      <Button
                        size="sm"
          ...
          {servers ? (
            <div className={styles.servers}>
              {servers
                ?.filter(
                  (server) =>
                    (server[serversFilterType] &&
                      server[serversFilterType]
                        .toLowerCase()
                        .includes(serversFilterText.toLowerCase())) ||
                    ...
                    <div className={styles.info}>
                      <div className={styles.text}>
                        ...
                        <Alert
                          ...
                          description={
                            <p>
                              Failed to select server; possibly not in server.
                            </p>
                          }
                          style={{
                            marginTop: "1rem",
                            display: serverAlerts[server.id] ? "flex" : "none",
                          }}
                        />
                      </div>
                      <div className={styles.buttons}>
            ...
            <Alert
              ...
              description={
                <p>
                  This content could not be accessed, perhaps you're missing
                  permissions.
                </p>
              }
            />
    ...
    },
    {
      ...
      content: selectedServer ? (
        <>
          ...
          >
            <Input
              ...
              onInput={(e) => setServerNameValue(e.target.value)}
            />
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={serverNameReasonValue}
                onInput={(e) => setServerNameReasonValue(e.target.value)}
          ...
          >
            <Input
              label="Days of inactivity to prune"
              value={serverPruneValue}
              onInput={(e) => setServerPruneValue(e.target.value)}
            />
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={serverPruneReasonValue}
                onInput={(e) => setServerPruneReasonValue(e.target.value)}
                label="Reason (optional)"
                customClass={styles.input}
              />
            </div>
          ...
          <Alert
            ...
            description={
              <p>
                Do not excessively use the <b>Prune</b> option.
              </p>
            }
          />
          ...
          >
            ...
            <div className={styles.buttons}>
              ...
              <Button
                ...
                customClass={styles.button}
                onClick={() => {
                  setServerPruneValue("7");
                  setServerPruneReasonValue("");
                  setServerPruneModalActive(true);
                }}
              />
              <Button
                size="sm"
              ...
              <Button
                ...
                onClick={() => {
                  setServerNameValue("");
                  setServerNameReasonValue("");
                  setServerRenameModalActive(true);
                }}
              />
            </div>
          </div>
          ...
          >
            <Input
              value={serverUnbanReasonValue}
              onInput={(e) => setServerUnbanReasonValue(e.target.value)}
              label="Reason (optional)"
              customClass={styles.input}
            />
          </Modal>
          ...
          >
            <Input
              value={serverInviteDeleteReasonValue}
              onInput={(e) => setServerInviteDeleteReasonValue(e.target.value)}
              label="Reason (optional)"
              customClass={styles.input}
            />
          </Modal>
              ...
              {selectedServerInfo?.bans && (
                <>
                  <Button
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setServerUnbanReasonValue("");
                      setServerUnbanModalActive(true);
                    }}
                  />
                  <div className={styles.filter}>
                    <Input
            ...
            <div className={styles.table}>
              <Table
                ...
                data={selectedServerInfo?.bans
                  ?.filter(
                    (ban) =>
                      ban.user[bansFilterType] &&
                      ban.user[bansFilterType]
                        .toLowerCase()
                        .includes(bansFilterText.toLowerCase()),
                  )
                  ?.map((ban) => ({
                    id: ban.user.id,
                    user:
          ...
          >
            <div className={styles.mobile}>
              ...
              <div className={styles.filter}>
                ...
                >
                  <option className={styles.option} value={"code"}>
                    Code
                  </option>
                  <option className={styles.option} value={"name"}>
                    Channel
              ...
              {selectedServerInfo?.invites && (
                <>
                  <Button
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setServerInviteDeleteReasonValue("");
                      setServerInviteDeleteModalActive(true);
                    }}
                  />
                  <div className={styles.filter}>
                    <Input
            ...
            <div className={styles.table}>
              <Table
                ...
                columns={
                  dimensions.width > 1000
                    ? inviteColumns
                    : inviteColumns.filter((c) =>
                        ["Code", "Channel", "Creator"].includes(c.name),
                      )
                }
                data={selectedServerInfo?.invites
                  ?.filter((invite) => {
                    if (invite[invitesFilterType])
                      return invite[invitesFilterType]
                        .toLowerCase()
                        .includes(invitesFilterText.toLowerCase());
                    if (invite.channel[invitesFilterType])
                      return invite.channel[invitesFilterType]
                        .toLowerCase()
                        .includes(invitesFilterText.toLowerCase());
                    if (invite.inviter[invitesFilterType])
                      return invite.inviter[invitesFilterType]
                        .toLowerCase()
                        .includes(invitesFilterText.toLowerCase());
                  })
                  ?.map((invite) => ({
                    ...
                    creator:
                      invite.inviter?.discriminator !== "0"
            ...
            <div className={styles.table}>
              <Table
                ...
                columns={
                  dimensions.width > 1000
                    ? templateColumns
                    : templateColumns.filter((c) =>
                        ["Code", "Name"].includes(c.name),
                      )
                }
                data={selectedServerInfo?.templates
                  ?.filter(
                    (template) =>
                      template[templatesFilterType] &&
                      template[templatesFilterType]
                        .toLowerCase()
                        .includes(templatesFilterText.toLowerCase()),
                  )
                  ?.map((template) => ({
                    id: template.code,
                    name: template.name,
                    ...
                    creator:
                      template.creator?.discriminator !== "0"
                        ? `${template.creator?.username}#${template.creator?.discriminator}`
        ...
        <Alert variant="info" description={<p>Selecting server...</p>} />
      ) : (
        <Alert
          variant="warning"
          description={<p>No server is currently selected.</p>}
        />
      ),
    },
    {
        ...
        <>
          ...
          >
            <Input
              ...
              label="Message"
              value={userDMValue}
              onInput={(e) => setUserDMValue(e.target.value)}
            />
            <div style={{ marginTop: "1rem" }}>
              <Input
                label="Amount"
                value={userDMAmountValue}
                onInput={(e) => setUserDMAmountValue(e.target.value)}
              />
            </div>
          </Modal>
          ...
          >
            <Input
              ...
              onInput={(e) => setUserNicknameValue(e.target.value)}
            />
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={userNicknameReasonValue}
                onInput={(e) => setUserNicknameReasonValue(e.target.value)}
                label="Reason (optional)"
                customClass={styles.input}
              />
            </div>
          ...
          >
            <Input
              label="Timeout (examples: 1s, 1m, 1h, 1d)"
              value={userTimeoutValue}
              onInput={(e) => setUserTimeoutValue(e.target.value)}
            />
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={userTimeoutReasonValue}
                onInput={(e) => setUserTimeoutReasonValue(e.target.value)}
                label="Reason (optional)"
                customClass={styles.input}
              />
            </div>
          ...
          >
            ...
            {selectedServerInfo?.roles ? (
              <div
                ...
                style={{
                  width: dimensions.width > 1000 ? "50rem" : "fit-content",
                }}
              >
                <Table
                  theme="dark"
                  columns={
                    dimensions.width > 1000
                      ? roleColumns
                      : roleColumns.filter((c) =>
                          ["Name", "Id"].includes(c.name),
                        )
                  }
                  data={selectedServerInfo?.roles
                    ...
                    ?.filter(
                      (role) =>
                        role[removeRolesFilterType] &&
                        role[removeRolesFilterType]
                          .toLowerCase()
                          .includes(removeRolesFilterText.toLowerCase()),
                    )
                    ?.map((role) => ({
                      id: role.id,
                      name: role.name,
            ...
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={userRemoveRoleReasonValue}
                onInput={(e) => setUserRemoveRoleReasonValue(e.target.value)}
                label="Reason (optional)"
                customClass={styles.input}
              />
            </div>
          ...
          >
              ...
              >
                <Table
                  ...
                  data={selectedServerInfo?.roles
                    ...
                    ?.filter(
                      (role) =>
                        role[addRolesFilterType] &&
                        role[addRolesFilterType]
                          .toLowerCase()
                          .includes(addRolesFilterText.toLowerCase()),
                    )
                    ?.map((role) => ({
                      id: role.id,
                      name: role.name,
            ...
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={userAddRoleReasonValue}
                onInput={(e) => setUserAddRoleReasonValue(e.target.value)}
                label="Reason (optional)"
                customClass={styles.input}
              />
            </div>
          ...
          >
            <Input
              value={userKickReasonValue}
              onInput={(e) => setUserKickReasonValue(e.target.value)}
              label="Reason (optional)"
              customClass={styles.input}
            />
          </Modal>
          ...
          >
            <Input
              label="Days to delete messages"
              value={userBanValue}
              onInput={(e) => setUserBanValue(e.target.value)}
            />
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={userBanReasonValue}
                onInput={(e) => setUserBanReasonValue(e.target.value)}
                label="Reason (optional)"
                customClass={styles.input}
              />
            </div>
          ...
          {!client?.bot && (
            <Alert
              ...
              description={
                <p>
                  You might be terminated for using the <b>DM</b> option on an
                  user account.
                </p>
              }
            />
            ...
            <div className={styles.mobile}>
              ...
              <div className={styles.filter}>
                ...
                >
                  ...
                  <option className={styles.option} value={"nick"}>
                    Nickname
                  </option>
                  <option className={styles.option} value={"id"}>
                    Id
              ...
              {selectedServerInfo?.members && (
                <>
                  <Button
                    ...
                    onClick={() => {
                      setUserDMValue("");
                      setUserDMAmountValue("1");
                      setUserDMModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setUserNicknameValue("");
                      setUserNicknameReasonValue("");
                      setUserNicknameModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    onClick={() => {
                      setUserTimeoutValue("");
                      setUserTimeoutReasonValue("");
                      setUserTimeoutModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setUserAddRoleReasonValue("");
                      setUserAddRoleModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setUserRemoveRoleReasonValue("");
                      setUserRemoveRoleModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setUserKickReasonValue("");
                      setUserKickModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    onClick={() => {
                      setUserBanValue("0");
                      setUserBanReasonValue("");
                      setUserBanModalActive(true);
                    }}
                  />
                  <div className={styles.filter}>
                    <Input
            ...
            <div className={styles.table}>
              <Table
                ...
                data={selectedServerInfo?.members
                  ...
                  ?.filter(
                    (member) =>
                      member[usersFilterType] &&
                      member[usersFilterType]
                        .toLowerCase()
                        .includes(usersFilterText.toLowerCase()),
                  )
                  ?.map((member) => ({
                    id: member.id,
                    user:
                    ...
                    joined: moment(member.joinedAt).format(
                      "YYYY-MM-DD HH:mm:ss",
                    ),
        ...
        <>
          ...
          >
            <Input
              multiLine={true}
              label="Message"
              value={channelMessageValue}
              onInput={(e) => setChannelMessageValue(e.target.value)}
            />
            <div style={{ marginTop: "1rem" }}>
              <Input
                label="Amount"
                value={channelMessageAmountValue}
                onInput={(e) => setChannelMessageAmountValue(e.target.value)}
              />
            </div>
          </Modal>
          ...
          >
            <Input
              label="Message limit (-1 for no limit)"
              value={channelPurgeValue}
              onInput={(e) => setChannelPurgeValue(e.target.value)}
            />
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={channelPurgeReasonValue}
                onInput={(e) => setChannelPurgeReasonValue(e.target.value)}
                label="Reason (optional)"
                customClass={styles.input}
              />
            </div>
          ...
          >
            <Input
              ...
              value={channelCreateInviteMaxAgeValue}
              onInput={(e) => setChannelCreateInviteMaxAgeValue(e.target.value)}
            />
            <div style={{ marginTop: "1rem" }}>
              <Input
                label="Max uses (number, optional)"
                value={channelCreateInviteMaxUsesValue}
                onInput={(e) =>
                  setChannelCreateInviteMaxUsesValue(e.target.value)
                }
              />
            </div>
            <div
            ...
            >
              <input
                ...
                type="checkbox"
                checked={channelCreateInviteTemporaryValue}
                onChange={(e) =>
                  setChannelCreateInviteTemporaryValue(e.target.checked)
                }
              />
            ...
            >
              <input
                className={styles.checkbox}
                type="checkbox"
                checked={channelCreateInviteUniqueValue}
                onChange={(e) =>
                  setChannelCreateInviteUniqueValue(e.target.checked)
                }
              />
            ...
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={channelCreateInviteReasonValue}
                onInput={(e) =>
                  setChannelCreateInviteReasonValue(e.target.value)
                }
                label="Reason (optional)"
                customClass={styles.input}
              />
          ...
          >
            <Input
              ...
              onInput={(e) => setChannelRenameValue(e.target.value)}
              label="Name"
              customClass={styles.input}
            />
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={channelRenameReasonValue}
                onInput={(e) => setChannelRenameReasonValue(e.target.value)}
                label="Reason (optional)"
                customClass={styles.input}
              />
            </div>
          ...
          >
            <Input
              label="Name"
              value={channelCreateNameValue}
              onInput={(e) => setChannelCreateNameValue(e.target.value)}
            />
            <div style={{ marginTop: "1rem" }}>
              <h3 className={styles.label}>Type</h3>
              <select
                className={styles.select}
                value={channelCreateTypeValue}
                onChange={(e) => setChannelCreateTypeValue(e.target.value)}
              >
                ...
                <option className={styles.option} value={13}>
                  Stage
                </option>
              </select>
            </div>
            <div style={{ marginTop: "1rem" }}>
              <Input
                label="Amount"
                value={channelCreateAmountValue}
                onInput={(e) => setChannelCreateAmountValue(e.target.value)}
              />
            </div>
            <div
              style={{ marginTop: "1rem" }}
            ...
            >
              <input
                className={styles.checkbox}
                type="checkbox"
                checked={channelCreateNSFWValue}
                onChange={(e) => setChannelCreateNSFWValue(e.target.checked)}
              />
              <h3 className={styles.label}>NSFW (optional)</h3>
            </div>
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={channelCreateReasonValue}
                onInput={(e) => setChannelCreateReasonValue(e.target.value)}
                label="Reason (optional)"
                customClass={styles.input}
              />
            </div>
          ...
          >
            <Input
              value={channelDeleteReasonValue}
              onInput={(e) => setChannelDeleteReasonValue(e.target.value)}
              label="Reason (optional)"
              customClass={styles.input}
            />
          </Modal>
                ...
                <>
                  <Button
                    ...
                    onClick={() => {
                      ...
                      setChannelMessageAmountValue("1");
                      setChannelMessageModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setChannelPurgeValue("-1");
                      setChannelPurgeReasonValue("");
                      setChannelPurgeModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setChannelCreateInviteMaxAgeValue("");
                      setChannelCreateInviteMaxUsesValue("");
                      setChannelCreateInviteTemporaryValue(false);
                      setChannelCreateInviteUniqueValue(false);
                      setChannelCreateInviteReasonValue("");
                      setChannelCreateInviteModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    onClick={() => {
                      ...
                      setChannelRenameReasonValue("");
                      setChannelRenameModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setChannelCreateNameValue("");
                      setChannelCreateTypeValue(0);
                      setChannelCreateAmountValue(1);
                      setChannelCreateNSFWValue(false);
                      setChannelCreateReasonValue("");
                      setChannelCreateModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setChannelDeleteReasonValue("");
                      setChannelDeleteModalActive(true);
                    }}
                  />
                  <div className={styles.filter}>
                    <Input
            ...
            <div className={styles.table}>
              <Table
                ...
                data={selectedServerInfo?.channels
                  ?.filter(
                    (channel) =>
                      channel[channelsFilterType] &&
                      channel[channelsFilterType]
                        .toLowerCase()
                        .includes(channelsFilterText.toLowerCase()),
                  )
                  ?.map((channel) => ({
                    id: channel.id,
                    name: channel.name,
      ...
      content: selectedServer ? (
        <>
          ...
          >
              ...
              >
                <Table
                  ...
                  data={selectedServerInfo?.members
                    ...
                    ?.filter(
                      (member) =>
                        member[addMembersFilterType] &&
                        member[addMembersFilterType]
                          .toLowerCase()
                          .includes(addMembersFilterText.toLowerCase()),
                    )
                    ?.map((member) => ({
                      id: member.id,
                      user:
            ...
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={roleAddMemberReasonValue}
                onInput={(e) => setRoleAddMemberReasonValue(e.target.value)}
                label="Reason (optional)"
                customClass={styles.input}
              />
            </div>
          ...
          >
                ...
                {selectedServerInfo?.members && (
                  <div className={styles.filter}>
                    <Input
                      ...
                      onInput={(e) =>
                        setRemoveMembersFilterText(e.target.value)
                      }
                    />
                    <select
                      className={`${styles.select} ${styles.filterSelect}`}
                      ...
                      onChange={(e) =>
                        setRemoveMembersFilterType(e.target.value)
                      }
                    >
                      <option className={styles.option} value={"username"}>
                        Username
              ...
              >
                <Table
                  ...
                  data={selectedServerInfo?.members
                    ...
                    ?.filter(
                      (member) =>
                        member[removeMembersFilterType] &&
                        member[removeMembersFilterType]
                          .toLowerCase()
                          .includes(removeMembersFilterText.toLowerCase()),
                    )
                    ?.map((member) => ({
                      id: member.id,
                      user:
            ...
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={roleRemoveMemberReasonValue}
                onInput={(e) => setRoleRemoveMemberReasonValue(e.target.value)}
                label="Reason (optional)"
                customClass={styles.input}
              />
            </div>
          ...
          >
            <Input
              value={roleRenameValue}
              onInput={(e) => setRoleRenameValue(e.target.value)}
              label="Name"
              customClass={styles.input}
            />
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={roleRenameReasonValue}
                onInput={(e) => setRoleRenameReasonValue(e.target.value)}
                label="Reason (optional)"
                customClass={styles.input}
              />
            </div>
          ...
          >
            <Input
              label="Name"
              value={roleCreateNameValue}
              onInput={(e) => setRoleCreateNameValue(e.target.value)}
            />
            <div style={{ marginTop: "1rem" }}>
              <Input
                label="Amount"
                value={roleCreateAmountValue}
                onInput={(e) => setRoleCreateAmountValue(e.target.value)}
              />
            </div>
            <div
              style={{ marginTop: "1rem" }}
            ...
            >
              <input
                className={styles.checkbox}
                type="checkbox"
                checked={roleCreateHoistValue}
                onChange={(e) => setRoleCreateHoistValue(e.target.checked)}
              />
            ...
            >
              <input
                className={styles.checkbox}
                type="checkbox"
                checked={roleCreateMentionableValue}
                onChange={(e) =>
                  setRoleCreateMentionableValue(e.target.checked)
                }
              />
            ...
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={roleCreateReasonValue}
                onInput={(e) => setRoleCreateReasonValue(e.target.value)}
                label="Reason (optional)"
                customClass={styles.input}
              />
            </div>
          ...
          >
            <Input
              value={roleDeleteReasonValue}
              onInput={(e) => setRoleDeleteReasonValue(e.target.value)}
              label="Reason (optional)"
              customClass={styles.input}
            />
          </Modal>
              ...
              {selectedServerInfo?.roles && (
                <>
                  <Button
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setRoleAddMemberReasonValue("");
                      setRoleAddMemberModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setRoleRemoveMemberReasonValue("");
                      setRoleRemoveMemberModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    onClick={() => {
                      setRoleRenameValue("");
                      setRoleRenameReasonValue("");
                      setRoleRenameModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setRoleCreateNameValue("");
                      setRoleCreateAmountValue(1);
                      setRoleCreateHoistValue(false);
                      setRoleCreateMentionableValue(false);
                      setRoleCreateReasonValue("");
                      setRoleCreateModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setRoleDeleteReasonValue("");
                      setRoleDeleteModalActive(true);
                    }}
                  />
                  <div className={styles.filter}>
                    <Input
            ...
            <div className={styles.table}>
              <Table
                ...
                data={selectedServerInfo?.roles
                  ...
                  ?.filter(
                    (role) =>
                      role[rolesFilterType] &&
                      role[rolesFilterType]
                        .toLowerCase()
                        .includes(rolesFilterText.toLowerCase()),
                  )
                  ?.map((role) => ({
                    id: role.id,
                    name: role.name,
        ...
        <>
          ...
          >
            <Input
              ...
              onInput={(e) => setEmojiRenameValue(e.target.value)}
              label="Name"
              customClass={styles.input}
            />
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={emojiRenameReasonValue}
                onInput={(e) => setEmojiRenameReasonValue(e.target.value)}
                label="Reason (optional)"
                customClass={styles.input}
              />
            </div>
          ...
          >
            <Input
              label="Name"
              value={emojiCreateNameValue}
              onInput={(e) => setEmojiCreateNameValue(e.target.value)}
            />
            <div style={{ marginTop: "1rem" }}>
              <Input
                label="Amount"
                value={emojiCreateAmountValue}
                onInput={(e) => setEmojiCreateAmountValue(e.target.value)}
              />
            </div>
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={emojiCreateReasonValue}
                onInput={(e) => setEmojiCreateReasonValue(e.target.value)}
                label="Reason (optional)"
                customClass={styles.input}
              />
            </div>
          ...
          >
            <Input
              value={emojiDeleteReasonValue}
              onInput={(e) => setEmojiDeleteReasonValue(e.target.value)}
              label="Reason (optional)"
              customClass={styles.input}
            />
          </Modal>
              ...
              {selectedServerInfo?.emojis && (
                <>
                  <Button
                    ...
                    onClick={() => {
                      setEmojiRenameValue("");
                      setEmojiRenameReasonValue("");
                      setEmojiRenameModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setEmojiCreateNameValue("");
                      setEmojiCreateAmountValue(1);
                      setEmojiCreateReasonValue("");
                      setEmojiCreateModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setEmojiDeleteReasonValue("");
                      setEmojiDeleteModalActive(true);
                    }}
                  />
                  <div className={styles.filter}>
                    <Input
            ...
            <div className={styles.table}>
              <Table
                ...
                data={selectedServerInfo?.emojis
                  ?.filter(
                    (emoji) =>
                      emoji[emojisFilterType] &&
                      emoji[emojisFilterType]
                        .toLowerCase()
                        .includes(emojisFilterText.toLowerCase()),
                  )
                  ?.map((emoji) => ({
                    id: emoji.id,
                    name: emoji.name,
        ...
        <>
          ...
          >
            <Input
              ...
              onInput={(e) => setStickerRenameValue(e.target.value)}
              label="Name"
              customClass={styles.input}
            />
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={stickerRenameReasonValue}
                onInput={(e) => setStickerRenameReasonValue(e.target.value)}
                label="Reason (optional)"
                customClass={styles.input}
              />
            </div>
          ...
          >
            <Input
              label="Name"
              value={stickerCreateNameValue}
              onInput={(e) => setStickerCreateNameValue(e.target.value)}
            />
            <div style={{ marginTop: "1rem" }}>
              <Input
                label="Emoji (e.g. 🔥)"
                value={stickerCreateEmojiValue}
                onInput={(e) => setStickerCreateEmojiValue(e.target.value)}
              />
            </div>
            <div style={{ marginTop: "1rem" }}>
              <Input
                label="Amount"
                value={stickerCreateAmountValue}
                onInput={(e) => setStickerCreateAmountValue(e.target.value)}
              />
            </div>
            <div style={{ marginTop: "1rem" }}>
              <Input
                value={stickerCreateReasonValue}
                onInput={(e) => setStickerCreateReasonValue(e.target.value)}
                label="Reason (optional)"
                customClass={styles.input}
              />
            </div>
          ...
          >
            <Input
              value={stickerDeleteReasonValue}
              onInput={(e) => setStickerDeleteReasonValue(e.target.value)}
              label="Reason (optional)"
              customClass={styles.input}
            />
          </Modal>
                ...
                <>
                  <Button
                    ...
                    onClick={() => {
                      ...
                      setStickerRenameReasonValue("");
                      setStickerRenameModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setStickerCreateNameValue("");
                      setStickerCreateEmojiValue("");
                      setStickerCreateAmountValue(1);
                      setStickerCreateReasonValue("");
                      setStickerCreateModalActive(true);
                    }}
                  />
                  <Button
                    size="sm"
                    ...
                    customClass={styles.button}
                    onClick={() => {
                      setStickerDeleteReasonValue("");
                      setStickerDeleteModalActive(true);
                    }}
                  />
                  <div className={styles.filter}>
                    <Input
            ...
            <div className={styles.table}>
              <Table
                ...
                data={selectedServerInfo?.stickers
                  ?.filter(
                    (sticker) =>
                      sticker[stickersFilterType] &&
                      sticker[stickersFilterType]
                        .toLowerCase()
                        .includes(stickersFilterText.toLowerCase()),
                  )
                  ?.map((sticker) => ({
                    id: sticker.id,
                    name: sticker.name,
      ...
      <div className={`${styles.title} ${styles.mainTitle}`}>
        <div className={styles.tabs}>
          {tabs.map((tab) => (
            <div
              key={tab.label}
              className={`${styles.tab} ${currentTab === tab.label.toLowerCase() && styles.active}`}
              onClick={() => {
      ...
      {client ? (
        <div className={styles.flex}>
          <div className={styles.content}>
            {
              (
                tabs.find((tab) => tab.label.toLowerCase() === currentTab) ||
                tabs.find((tab) => tab.label.toLowerCase() === "servers")
              ).content
            }
          </div>
          <div className={styles.logs}>
            ...
            <div className={styles.text}>
              {logs.sort((a, b) => b.id - a.id).map(({ log }) => log)}
            </div>
          </div>
        </div>
      ) : (
        <Alert
          variant="warning"
          description={<p>No instance is currently connected.</p>}
          style={{ marginTop: "1rem" }}
        />
      )}
    </>
